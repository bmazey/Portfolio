...
namespace java {
  namespace lang {
    ...
    struct __Character {
      ...
      // The initializer (for executing the default constructor of a superclass)
      static Character init(Character);
  ...
  namespace lang {
    ...
    class ArrayIndexOutOfBoundsException : public IndexOutOfBoundsException {
      public:
        ...
        {
          std::cerr << "Exception: ArrayIndexOutOfBoundsException" << std::endl;
        }
    };
  }
}
...
namespace __rt {
  ...
  struct Array {
    Array_VT<T>* __vptr;
    const int32_t length_var;
    T* __data;

    // The constructor (defined inline).
    Array(const int32_t length)
      : __vptr(&__vtable), length_var(length), __data(new T[length]()) {
      if(length < 0)
        throw java::lang::NegativeArraySizeException();
    }

    // The initializer (for executing the default constructor of a superclass)
    static Ptr<Array<T>, array_policy> init(Ptr<Array<T>, array_policy> __this)
    {
      java::lang::__Object::init(__this);

      return __this;
    }

    static Ptr<Array<T>, array_policy> init(Ptr<Array<T>, array_policy> __this, const int32_t length)
    {
      java::lang::__Object::init(__this);
  ...
  template <typename T>
  void checkIndex(Ptr<Array<T>, array_policy> array, int32_t index) {
    if (0 > index || index >= array->length_var) {
      throw java::lang::ArrayIndexOutOfBoundsException();
    }
  }
  ...
  void checkStore(Ptr<Array<T>, array_policy> array, U object) {
    if (null() != (java::lang::Object) object) {
